...
router.get('/authenticate', function (req, res, next) {
    var sess = req.session;
    authenticator.login(req.query.email, req.query.pwd).then(function (result) {
        if (result) {
...
router.post('/register', function (req, res, next) {
    ...
    var result = clientAccountRepository.createAccount(req.body.email, req.body.pwd).catch(function (error) {
        console.log(error);
    });

    if (!result) res.render('register', { error: 'Email already exists' });else res.redirect('/login');
});
